package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      private var var_2138:MarketplaceItemStats;
      ...
      {
         if(!var_346 || !_catalog || !_catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = var_346.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         showConfirmation(const_1577,_loc2_);
      }
      ...
      {
         if(param1.furniCategoryId != var_1842 || param1.furniTypeId != var_1838)
         {
            return;
         }
         var_2138 = param1;
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            var_346.remove(parser.requestedOfferId);
            _visualization.listUpdatedNotify();
            if(_windowManager != null)
            {
         ...
         {
            updateItem = var_346.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               var_346.add(parser.offerId,updateItem);
            }
            var_346.remove(parser.requestedOfferId);
            showConfirmation(const_1571,updateItem);
            _visualization.listUpdatedNotify();
         }
      ...
      public function redeemSoldOffers() : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc1_:Array = var_703.getKeys();
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = var_703.getValue(_loc2_);
            if(_loc3_.status == MarketPlaceOfferState.const_357)
            {
               var_703.remove(_loc2_);
            }
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         var_2137 = _loc3_.creditsWaiting;
         var_703 = new Map();
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_703.add(_loc4_.offerId,_loc5_);
         }
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_346.add(_loc4_.offerId,_loc6_);
         }
         if(_visualization == null)
         {
            return;
      ...
      {
         requestOffers(var_2136,var_2133,var_2135,var_2134);
      }
