package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_417 >= var_307.length)
         {
            return "";
         }
         var petColor:IPetColor = var_307[var_417] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_590[var_1543] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_417 = param1.index;
         if(var_417 < 0 || var_417 > var_307.length)
         {
            var_417 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_555.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         var_887 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_307 = [];
         var_590 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_887);
               if(_loc11_)
               {
                  var_590 = _loc11_.breeds;
                  var_307 = _loc11_.colors;
                  if(var_307 == null)
                  {
                     var_307 = [];
                  }
         ...
         while(_loc6_ < var_590.length)
         {
            ...
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_887,_loc12_.id),getRaceLocalizationKey(var_887,_loc12_.id)));
            _loc6_++;
         }
         var_555.add(var_887,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_849,onApproveNameResult);
         _loc5_ = var_555.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_307)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_555.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_417 >= 0 && var_417 < var_307.length)
         {
            _loc7_ = var_307[var_417] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_590[var_1543] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_887,_loc3_.id,_loc2_,AvatarScaleType.const_50);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_293,AvatarAction.const_625);
