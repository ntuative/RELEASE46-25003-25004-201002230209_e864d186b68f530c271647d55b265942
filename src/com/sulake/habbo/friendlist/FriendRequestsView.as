package com.sulake.habbo.friendlist
{
   ...
   public class FriendRequestsView implements ITabView, IFriendRequestsView
   {
      ...
      private function onAcceptAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            if(_loc4_.state != FriendRequest.const_301 && _loc4_.state != FriendRequest.const_297)
            {
      ...
      {
         var_1399 = IContainerButtonWindow(param1.findChildByName("accept_all_but"));
         var_1400 = IContainerButtonWindow(param1.findChildByName("reject_all_but"));
         var_1400.procedure = onDismissAllButtonClick;
         var_1399.procedure = onAcceptAllButtonClick;
         refreshButtons();
      }
      ...
      private function onDismissAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         for each(_loc4_ in _friendList.friendRequests.requests)
         {
            if(_loc4_.state != FriendRequest.const_301 && _loc4_.state != FriendRequest.const_297)
            {
               _loc4_.state = FriendRequest.const_297;
               refreshRequestEntry(_loc4_);
               if(_friendList && false)
               {
                  _friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_247,_loc4_.requestId));
               }
            }
         }
         refresh();
      ...
      public function refreshShading(param1:FriendRequest, param2:Boolean) : void
      {
         ...
         param1.view.color = _friendList.method_2.getRowShadingColor(FriendListTab.const_79,param2);
      ...
      public function refreshRequestEntry(param1:FriendRequest) : void
      {
         ...
         if(param1.state == FriendRequest.const_132)
         {
         ...
         else if(param1.state == FriendRequest.const_297)
         {
            _friendList.refreshText(_loc2_,"info_text",true,"${friendlist.request.declined}");
         }
         else if(param1.state == FriendRequest.const_956)
         {
      ...
      private function refreshButtons() : void
      {
         ...
         setEnabled(var_1399,_loc1_);
         setEnabled(var_1400,_loc1_);
      }
      ...
      public function declineRequest(param1:int) : void
      {
         ...
         _loc2_.state = FriendRequest.const_297;
         var _loc3_:DeclineBuddyMessageComposer = new DeclineBuddyMessageComposer();
         ...
         if(_friendList && false)
         {
            _friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_247,param1));
         }
      }
   }
}
