package com.sulake.habbo.navigator.roomsettings
{
   ...
   public class RoomSettingsCtrl
   {
      ...
      private var var_799:TextFieldManager;
      ...
      private var var_390:IRoomSettingsCtrlOwner;
      ...
      {
         close();
         this.var_390.roomSettingsRefreshNeeded();
      }
      ...
      private function onConfirmRoomDelete(param1:WindowMouseEvent, param2:int) : void
      {
         ...
         close();
         var_390.roomSettingsRefreshNeeded();
         if(_navigator.data.guestRoomSearchResults != null)
         {
            _loc3_ = _navigator.data.guestRoomSearchResults;
            _navigator.mainViewCtrl.startSearch(_navigator.tabs.getSelected().id,_loc3_.searchType,_loc3_.method_6);
         }
      ...
      {
         Logger.log("[RoomSettingsCtrl.onDeleteButtonClick] " + var_391);
         _navigator.registerParameter("navigator.roomsettings.deleteroom.confirm.message","room_name",var_84.name);
         var _loc2_:ConfirmDialogView = new ConfirmDialogView(IFrameWindow(_navigator.getXmlWindow("ros_room_delete_confirm")),this,onConfirmRoomDelete,[var_391]);
         _loc2_.show();
      }
      ...
      {
         return IWindowContainer(_navigator.windowManager.createWindow(param1,"",HabboWindowType.const_56,HabboWindowStyle.const_36,HabboWindowParam.const_64,new Rectangle(0,0,100,20)));
      }
      ...
      public function refresh(param1:IWindowContainer) : void
      {
         ...
         var_1440.visible = _inRoom;
         var_1438.visible = _inRoom;
         var_1865.visible = var_460;
         var_1866.visible = !var_460;
         if(var_460)
         {
            var_985.visible = true;
            var_1191.visible = false;
            refreshFlatControllers();
            var_985.height = Util.getLowestPoint(this.var_985) + 4;
         }
         else
         {
            populateForm();
            ...
            var_1196.visible = _inRoom;
            _navigator.thumbRenderer.refreshThumbnail(IWindowContainer(var_1196.findChildByName("picframe")),_navigator.data.enteredGuestRoom.thumbnail,false);
            var_983.height = Util.getLowestPoint(this.var_983) + 4;
         }
         var_1439.text = !!var_460 ? "${navigator.roomsettings.tobasicsettings}" : "${navigator.roomsettings.toadvancedsettings}";
         Util.moveChildrenToColumn(_window,[var_1440.name,var_1196.name,var_983.name,var_985.name,var_1191.name,var_1438.name],0,0);
      ...
      {
         this.var_391 = param1;
         _navigator.send(new GetRoomSettingsMessageComposer(var_391));
         if(var_981 != null)
         {
            var_981.visible = false;
         }
      }
      ...
      private function onSaveButtonClick(param1:WindowMouseEvent) : void
      {
         ...
         switch(_loc4_.name)
         {
            ...
            case "doormode_password":
               ...
               break;
            default:
               _loc2_.doorMode = RoomSettingsFlatInfo.const_424;
         }
         if(_loc2_.doorMode == RoomSettingsFlatInfo.const_122)
         {
            _loc8_ = var_982.getText();
            _loc9_ = var_1192.getText();
            if(_loc8_ != _loc9_)
            {
               this.var_982.clearErrors();
               this.var_1192.displayError("${navigator.roomsettings.invalidconfirm}");
               return;
            }
            if(_loc8_ != "")
            {
         ...
         _loc2_.allowPets = var_1193.isSelected;
         _loc2_.allowFoodConsume = var_1195.isSelected;
         _loc2_.allowWalkThrough = var_1190.isSelected;
         _loc2_.tags = new Array();
         addTag(var_804,_loc2_.tags);
         addTag(var_802,_loc2_.tags);
         this.clearErrors();
         this.var_984 = _loc2_.roomId;
      ...
      private function onRemoveAllFlatCtrlsClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         _navigator.send(new RemoveAllRightsMessageComposer(this.var_391));
      }
      
      public function onRoomSettingsSaved(param1:int) : void
      {
         if(param1 != this.var_391 || var_984 < 1)
         {
            return;
         }
         close();
      ...
      public function onRoomSettingsSaveError(param1:int, param2:int, param3:String) : void
      {
         ...
         var_984 = 0;
         if(param2 == RoomSettingsSaveErrorMessageParser.const_1332)
         {
            this.var_799.displayError("${navigator.roomsettings.roomnameismandatory}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1231)
         {
            this.var_799.displayError("${navigator.roomsettings.unacceptablewords}");
         }
         ...
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1340)
         {
         ...
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1133)
         {
            this.var_982.displayError("${navigator.roomsettings.passwordismandatory}");
         }
         else
         {
            this.var_799.displayError("Update failed: error " + param2);
         }
      }
      ...
      private function prepareWindow(param1:IWindowContainer) : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_734,onDoorModePasswordSelect);
         _loc2_.addEventListener(WindowEvent.const_834,onDoorModePasswordUnselect);
         ...
         this.var_799 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("room_name")),60);
         ...
         this.var_802 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("tag2")),30);
         this.var_982 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password")),30);
         this.var_1192 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password_confirm")),30);
         this.var_1193 = ICheckBoxWindow(_window.findChildByName("allow_pets_checkbox"));
         this.var_1195 = ICheckBoxWindow(_window.findChildByName("allow_foodconsume_checkbox"));
         this.var_1190 = ICheckBoxWindow(_window.findChildByName("allow_walk_through_checkbox"));
         var_1440 = IWindowContainer(_window.findChildByName("header_container"));
         var_1196 = IWindowContainer(_window.findChildByName("thumbnail_container"));
         var_983 = IWindowContainer(_window.findChildByName("basic_settings_container"));
         var_985 = IWindowContainer(_window.findChildByName("advanced_settings_container"));
         var_1438 = IWindowContainer(_window.findChildByName("switch_view_container"));
         var_1191 = IWindowContainer(_window.findChildByName("footer_container"));
         var_255 = IWindowContainer(_window.findChildByName("flat_controllers_container"));
         var_981 = IWindowContainer(_window.findChildByName("password_container"));
         var_1866 = ITextWindow(_window.findChildByName("basic_caption"));
         var_1865 = ITextWindow(_window.findChildByName("advanced_caption"));
         var_1439 = ITextWindow(_window.findChildByName("switch_view_text"));
         var_1439.procedure = onSwitchView;
         var_981.visible = false;
         if(!_inRoom)
         {
